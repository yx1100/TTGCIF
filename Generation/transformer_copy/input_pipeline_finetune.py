...
batch_size = 10
fine_tune_rate = 0.1  # todo 7 这里会影响到evaluate

print('-----Fine-tune Input_pipeline Start-----')

target_domain = 'Kindle_Store'  # todo 8 修改目标域

read_file_path = '../../datasets/8_dataset_final_mmd/dataset_final_mmd_toys/' + target_domain + '.pickle'  # todo 8 修改目标域文件夹
# read_file_path = '../../datasets/5_dataset_final/' + target_domain + '.pickle'  # todo no_mmd+maml 修改目标域文件夹
...
def gen(d_path):
    ...
    contents = contents[:fine_tune_size]
...
for _, batch_content in enumerate(dataset):
    dataset_fine_tune.append(batch_content)

print('The batch size of Fine-tune dataset:', batch_size)
print('The number of batch in Fine-tune dataset:', len(dataset_fine_tune))

print('-----Fine-tune Input Pipeline End-----')
