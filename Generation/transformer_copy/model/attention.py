...
def scaled_dot_product_attention(q, k, v, mask):
    ...
    # 遮挡（mask, value=1）与 -1e9（接近于负无穷）相乘。目标是将这些单元归零，因为 softmax 的较大负数输入在输出中接近于零。
    # 遮挡(mask)一批序列中所有的填充标记（pad tokens）。这确保了模型不会将填充作为输入。该 mask 表明填充值 0([PAD]) 出现的位置：在这些位置 mask 输出 1，否则输出 0。
